(function () {
    ...
    DYC.factory('httpInterceptor', function($q, $log, $rootScope, dialog, event, loginModal, httpConfig, progressBar) {
        function handleErrMsg(err) {
            return (httpErrorMsg[err.status] || '请求失败！') + ' ' + err.config.url;
        }
        ...
        var responseInterceptor = {
            request: function(config) {
                ...
                var windowLocationHost = rcpAid.getHost().host;
                if (windowLocationHost.indexOf('admin') > -1) {
                    windowLocationHost = '*';
                }
            ...
            requestError: function(response) {
                ...
                if (response.config.option.requestErrorMsg) {
                    if (typeof response.config.option.errorMsg === 'function') {
                        ...
                        dialog.alert(handleErrMsg(response));
                    }
                }
    ...
    DYC.config(function($httpProvider) {
