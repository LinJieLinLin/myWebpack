require('./lazy-img.scss');
let templateUrl = require('./lazy-img.html');
if (!window.DIR) {
    window.DIR = angular.module('DIR', []);
}
/**
 * @author linj
 * src:图片地址,传则会覆盖c.url的值
 * c:{
 *  url: 图片地址，
 *  id: 监听滚动元素id
 * }
 */
DIR.directive('lazyImg', function($document) {
    return {
        ...
        controller: function($scope, $element, $attrs, lazy) {
                ...
                $scope.listener = function(argEle) {
                    lazy.listener({
                        id: c.id || '',
                        node: argEle,
                        cbFn: function() {
...
DIR.factory('lazy', function($timeout, $window) {
    let factory = {
        /**
         * 监听事件
        ...
        listener: function(args) {
            ...
            function handler() {
                ...
                let wt = w.scrollTop;
                let wh = wt + w.offsetHeight;
                let et = rcpAid.safe(e.offsetParent, 'offsetTop');
                let eh = et + e.offsetHeight;
                console.log(et);
                // 判断是否在可视范围内
                let tf = et >= wt && et <= wh;
                // 判断是否在顶部
        ...
        dectroy: (argId) => {
            let w;
            if (argId) {
                w = document.getElementById(argId);
            } else {
                w = window;
            }
            angular.element(w).unbind('scroll');
            angular.element(w).unbind('resize');
        },
        /**
         * 实例
