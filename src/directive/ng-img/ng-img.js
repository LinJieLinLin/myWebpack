require('./ng-img.scss');
let templateUrl = require('./ng-img.html');
if (!window.DIR) {
    ...
    return {
        ...
        controller: function($scope, $element, $compile, $timeout) {
            try {
                ...
                let pushImg = function() {
                    if ($scope.isPushImg) {
                        return;
                    }
                    let temDefault = $scope.default || '',
                        imgClass = $scope.imgClass || '',
                        title = $scope.title || '';
                    let img = '<img ng-src="{{img}}" default="' + temDefault + '" class="ng-img ' + imgClass +
                        '" title="' + title +
                        '" style="visibility: hidden;" >';
                    $scope.isPushImg = true;
                ...
                $scope.init = function() {
                    ...
                    $scope.$watch('c', function(v, o) {
                        if (v && v !== o) {
                            angular.forEach($scope.c, function(value, key) {
                                $scope[key] = value;
                            });
                            if (!$scope.img) {
                                $scope.img = $scope.default;
                            }
                            pushImg();
                        }
                    });
